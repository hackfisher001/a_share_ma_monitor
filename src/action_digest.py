...
def action_summary_markdown(state: AlertState, ledger: TradeLedger) -> str:
    """Unfinished commitments, or nothing at all.

    Returns an empty string when nothing is pending. Only swing-T ever created
    these reminders, and it is off, so an unconditional card would say
    「无需操作」 every single day — a daily push that carries no information is
    worse than no push. Legacy reminders left in an older state file still show.
    """
    pending = state.pending_actions()
    if not pending:
        return ""
    lines = [f"**待你确认：{len(pending)} 件**"]
    ...
    if len(pending) > 3:
        lines.append(f"- 还有 {len(pending) - 3} 件待确认。")
